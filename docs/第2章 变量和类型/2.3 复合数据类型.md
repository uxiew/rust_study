...
Number里面要么存储的是i32，要么存储的是f32，它存储数据需要的空间应该是`max(sizeof(i32), sizeof(f32))=max(4 byte, 4 byte)=4 byte`。而它总共占用的内存是 8 byte，多出来的 4 byte 就是用于保存类型标记的。之所以用 4 byte，是为了内存对齐。
